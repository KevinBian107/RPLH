...
def run_exp(
    ...
    Saving_path_result = (
        Saving_path
        + f"/env_pg_state_{pg_row_num}_{pg_column_num}/pg_state{iteration_num}/{cen_decen_framework}{dialogue_history_method}_{model_name}"
    )
    ...
    for index_query_times in range(query_time_limit):  # The upper limit of calling LLMs
        ...
        if cen_decen_framework in ("DMAS"):
            ...
            if cen_decen_framework in ("CMAS", "HMAS-1", "HMAS-1-fast", "HMAS-2"):
                # -----------------------------------------PROMPT-----------------------------------------#
                user_prompt_1 = input_prompt_1_func_total(
                    state_update_prompt,
                    response_total_list,
                    pg_state_list,
                    dialogue_history_list,
                ...
                messages = message_construct_func(
                    [user_prompt_1], [], "_w_all_dialogue_history"
                )
            ...
            # -----------------------------------------RESPONSE-----------------------------------------#
            initial_response, token_num_count = LLaMA_response(
                messages, model_name
            )  # 'gpt-4' or 'gpt-3.5-turbo-0301' or 'gpt-4-32k' or 'gpt-3' or 'gpt-4-0613'
            ...
            if match:
                ...
                if response[0] == "{" and response[-1] == "}":
                    ...
                    token_num_count_list = (
                        token_num_count_list + token_num_count_list_add
                    )
                ...
                for local_agent_row_i in range(pg_row_num):
                    for local_agent_column_j in range(pg_column_num):
                        if (
                            ...
                            local_reprompt = (
                                input_prompt_local_agent_HMAS2_dialogue_func(
                                    state_update_prompt_local_agent,
                                    state_update_prompt_other_agent,
                                    response,
                                    response_total_list,
        ...
        try:
            system_error_feedback, pg_dict_returned = action_from_response(
                pg_dict, original_response_dict
            )
...
for pg_row_num, pg_column_num in [(2, 2), (2, 4), (4, 4), (4, 8)]:
    ...
    for iteration_num in range(2):
        print("-------###-------###-------###-------")
        print(
            f"Row num is: {pg_row_num}, Column num is: {pg_column_num}, Iteration num is: {iteration_num}\n\n"
        )
