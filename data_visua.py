...
print("-------#####------------#####------------#####--------")
Env_action_time_list_best_total = []
API_query_time_list_best_total = []
...
for pg_row_num, pg_column_num in [(2, 2), (2, 4), (4, 4), (4, 8)]:
    ...
    for iteration_num in range(10):
        Env_action_time_list.append(1e10)
        API_query_time_list.append(1e10)
        token_num_list.append(1e10)
        for cen_decen_framework, dialogue_history_method in candidate_list:
            ...
            with open(
                saving_path
                + f"env_pg_state_{pg_row_num}_{pg_column_num}/pg_state{iteration_num}/{cen_decen_framework}{dialogue_history_method}/success_failure.txt",
                "r",
            ) as file:
            ...
            if first_line == "success":
                with open(
                    saving_path
                    + f"env_pg_state_{pg_row_num}_{pg_column_num}/pg_state{iteration_num}/{cen_decen_framework}{dialogue_history_method}/env_action_times.txt",
                    "r",
                ) as file:
                ...
                with open(
                    saving_path
                    + f"env_pg_state_{pg_row_num}_{pg_column_num}/pg_state{iteration_num}/{cen_decen_framework}{dialogue_history_method}/token_num_count.txt",
                    "r",
                ) as file:
                    numbers = [float(line.strip()) for line in file.readlines()]
    ...
    print(
        f"Row num: {pg_row_num}, Column num: {pg_column_num}, Environment action times, {Env_action_time_list}"
    )
    print(
        f"Row num: {pg_row_num}, Column num: {pg_column_num}, API query times, {API_query_time_list}"
    )
    print(
        f"Row num: {pg_row_num}, Column num: {pg_column_num}, Consuming of token num, {token_num_list}"
    )
    print("\n")
...
for cen_decen_framework, dialogue_history_method in candidate_list:
        ...
        for iteration_num in range(4):
            ...
            if first_line == "success":
                ...
                if Env_action_time_list_best_total[index][iteration_num] < 1e10:
                    ...
                    Env_action_time_cost += (
                        numbers[0]
                        / Env_action_time_list_best_total[index][iteration_num]
                    )
                ...
                if API_query_time_list_best_total[index][iteration_num] < 1e10:
                    ...
                    API_query_time_cost += (
                        len(numbers)
                        / API_query_time_list_best_total[index][iteration_num]
                    )
                if token_num_list_best_total[index][iteration_num] < 1e10:
                    # print(np.sum(numbers)/token_num_list_best_total[index][iteration_num])
                    token_num_cost += (
                        np.sum(numbers)
                        / token_num_list_best_total[index][iteration_num]
                    )
        print(
            f"Row num: {pg_row_num}, Column num: {pg_column_num}, {cen_decen_framework}{dialogue_history_method}"
        )
    ...
    print("\n")
    print(
        f"success_rate: {success_rate_list}, {np.sum(success_rate_list)/len(success_rate_list)}"
    )
    print(
        f"Env_action_time_cost: {Env_action_time_list}, {np.sum(Env_action_time_list) / len(Env_action_time_list)}"
    )
    print(
        f"API_query_time_cost: {API_query_time_list}, {np.sum(API_query_time_list) / len(API_query_time_list)}"
    )
    print(
        f"token_num_cost: {token_num_list}, {np.sum(token_num_list) / len(token_num_list)}"
    )
