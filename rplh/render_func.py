...
def parse_action(action):
    ...
    if len(details) == 2:
        ...
        target = tuple(map(float, re.findall(r"\[([\d\.\-, ]+)\]", target)[0].split(",")))
...
def map_df(states):
    ...
    for center, items in states.items():
        ...
        for i, item in enumerate(targets):
            ...
            x_offset = max(-max_offset, min(x_offset, max_offset))  # Clamp within box bounds
...
def apply_action(box_map, actions):
    ...
    for position, action in actions.items():
        ...
        if "target" in target:  # Remove both moving item and target
            target_item = next(
                (item for item in updated_box_map.get(center, []) if f"{item[0]}_{item[1]}" == target), None
            )
            if target_item:
                updated_box_map[center].remove(target_item)
            
            moving_item_obj = next(
                (item for item in updated_box_map.get(center, []) if f"{item[0]}_{item[1]}" == moving_item), None
            )
            if moving_item_obj:
...
def render_map(initial_map, action_list):
    ...
    for state in state_list:
        ...
        hover_text = df.apply(lambda row: 
                                    f"Type: {row['type']}<br>Color: {row['color']}<br>X: {row['x']}<br>Y: {row['y']}", 
                                    axis=1)
        ...
        fig.update_layout(
            title=dict(
            text=f"Box Item Plot with state {iteration}" if iteration < 1 
        else f"Box Item Plot with state {iteration}<br>action {action}",  # Use the formatted title
